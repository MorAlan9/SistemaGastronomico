...
import javafx.fxml.FXMLLoader; // Import necesario
import javafx.fxml.Initializable;
import javafx.scene.Parent; // Import necesario
import javafx.scene.Scene; // Import necesario
import javafx.scene.chart.PieChart;
import javafx.scene.control.*;
import javafx.scene.control.cell.PropertyValueFactory;
import javafx.scene.input.KeyCode; // Import necesario
import javafx.scene.input.KeyCodeCombination; // Import necesario
import javafx.scene.layout.GridPane;
import javafx.stage.Stage; // Import necesario
import javafx.util.Pair;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.ApplicationContext; // Import necesario
...
public class VentasController implements Initializable {
    ...
    @Autowired private ApplicationContext context; // Necesario para volver al menú
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // --- AGREGADO: F9 PARA VOLVER AL MENÚ ---
        Platform.runLater(() -> {
            if (tablaVentas.getScene() != null) {
                tablaVentas.getScene().getAccelerators().put(
    ...
    // ==========================================
    // ESTADÍSTICAS Y SALDOS (SIN CAMBIOS)
    // ==========================================
    ...
    // ==========================================
    // ARQUEO DE MOZO (SIN CAMBIOS)
    // ==========================================
    @FXML
    void abrirReporteMozo() {
        List<String> mozos = tablaVentas.getItems().stream().map(VentaModel::getMozo).distinct().filter(m -> !"-".equals(m) && !"Sin Mozo".equals(m)).collect(Collectors.toList());
        ...
        dialog.setTitle("Arqueo Individual"); dialog.setHeaderText("Seleccione Mozo:"); dialog.setContentText("Mozo:");
    ...
    void accionCerrarCaja(ActionEvent event) {
        ...
        alert.setTitle("Cierre Z"); alert.setHeaderText("Confirmar Cierre");
        alert.showAndWait().ifPresent(r -> { if(r == ButtonType.OK) { mostrarDialogoFijo("EGRESO", "Cierre", "Retiro Final"); cajaAbierta = false; actualizarEstadoVisual(); } });
    }
    ...
    private void mostrarDialogoFijo(String t, String c, String d) { TextInputDialog td = new TextInputDialog("0"); td.setTitle(t); td.setHeaderText(c); td.showAndWait().ifPresent(m -> guardarMovimiento(t, c, d, new BigDecimal(m.replace(",", ".")))); }
    private void mostrarDialogo(String tipo, String[] cats) { Dialog<Pair<String,String>> d = new Dialog<>(); d.setTitle(tipo); d.getDialogPane().getButtonTypes().addAll(ButtonType.OK, ButtonType.CANCEL); GridPane g = new GridPane(); g.setHgap(10); g.setVgap(10); g.setPadding(new Insets(20)); ComboBox<String> cb = new ComboBox<>(FXCollections.observableArrayList(cats)); cb.getSelectionModel().selectFirst(); TextField tD = new TextField(); TextField tM = new TextField(); g.addRow(0, new Label("Cat:"), cb); g.addRow(1, new Label("Det:"), tD); g.addRow(2, new Label("$:"), tM); d.getDialogPane().setContent(g); Platform.runLater(tM::requestFocus); d.setResultConverter(b -> b==ButtonType.OK ? new Pair<>(cb.getValue()+"|"+tD.getText(), tM.getText()) : null); d.showAndWait().ifPresent(r -> { try { guardarMovimiento(tipo, r.getKey().split("\\|")[0], r.getKey().split("\\|")[1], new BigDecimal(r.getValue().replace(",", "."))); } catch(Exception e) { mostrarAlerta("Error", "Monto inválido"); } }); }
    ...
    private void mostrarAlerta(String titulo, String contenido) { Alert a = new Alert(Alert.AlertType.INFORMATION); a.setTitle(titulo); a.setHeaderText(null); a.setContentText(contenido); a.show(); }
    private String normalizarNombre(String raw) { if (raw == null) return "Efectivo"; String s = raw.toLowerCase(); if (s.contains("tarjeta") || s.contains("débito") || s.contains("crédito")) return "Tarjeta"; if (s.contains("qr") || s.contains("mp") || s.contains("mercado")) return "QR / MP"; if (s.contains("efectivo")) return "Efectivo"; if (s.contains("apertura")) return "Apertura"; return raw.substring(0, 1).toUpperCase() + raw.substring(1).toLowerCase(); }
    private void actualizarGraficoPie(PieChart c, Map<String, ? extends Number> d) { if(c==null)return; c.getData().clear(); c.setLabelsVisible(false); c.setLegendSide(Side.RIGHT); d.forEach((k,v)->c.getData().add(new PieChart.Data(k,v.doubleValue()))); for(PieChart.Data dt:c.getData()){ Tooltip.install(dt.getNode(),new Tooltip(dt.getName()+": "+dt.getPieValue())); dt.getNode().setOnMouseEntered(e->{dt.getNode().setScaleX(1.1);dt.getNode().setScaleY(1.1);}); dt.getNode().setOnMouseExited(e->{dt.getNode().setScaleX(1.0);dt.getNode().setScaleY(1.0);});} }
